import json
import gzip
import zlib

# --- 标准 Rijndael-256 纯 Python 实现 ---
SBOX = [
    
]

INV_SBOX = [0] * 256
for i, x in enumerate(SBOX):
    INV_SBOX[x] = i

def xtime(a):
    return ((a << 1) ^ 0x1B) & 0xFF if (a & 0x80) else (a << 1)

def mul(a, b):
    res = 0
    for _ in range(8):
        if b & 1:
            res ^= a
        a = xtime(a)
        b >>= 1
    return res

def get_rcon(n):
    c = 1
    for _ in range(n - 1):
        c = xtime(c)
    return c

class Rijndael256:
    def __init__(self, key):
        self.Nb = 8  # 256 bits block = 8 words
        self.Nk = 8  # 256 bits key = 8 words
        self.Nr = 14 # 14 rounds
        self.w = self._key_expansion(key)

    def _key_expansion(self, key):
        w = [list(key[i*4:(i+1)*4]) for i in range(self.Nk)]
        i = self.Nk
        while len(w) < self.Nb * (self.Nr + 1):
            temp = list(w[i - 1])
            if i % self.Nk == 0:
                temp = temp[1:] + temp[:1]
                temp = [SBOX[x] for x in temp]
                temp[0] ^= get_rcon(i // self.Nk)
            elif self.Nk > 6 and i % self.Nk == 4:
                temp = [SBOX[x] for x in temp]
            w.append([a ^ b for a, b in zip(w[i - self.Nk], temp)])
            i += 1
        return w

    def decrypt_block(self, block):
        state = [[block[r + 4 * c] for c in range(self.Nb)] for r in range(4)]
        
        for r in range(4):
            for c in range(self.Nb):
                state[r][c] ^= self.w[self.Nr * self.Nb + c][r]

        for round_num in range(self.Nr - 1, 0, -1):
            shifts = [0, 1, 3, 4]
            new_state = [[0]*self.Nb for _ in range(4)]
            for r in range(4):
                s = shifts[r]
                for c in range(self.Nb):
                    new_state[r][(c + s) % self.Nb] = state[r][c]
            state = new_state

            for r in range(4):
                for c in range(self.Nb):
                    state[r][c] = INV_SBOX[state[r][c]]

            for r in range(4):
                for c in range(self.Nb):
                    state[r][c] ^= self.w[round_num * self.Nb + c][r]

            for c in range(self.Nb):
                a0, a1, a2, a3 = state[0][c], state[1][c], state[2][c], state[3][c]
                state[0][c] = mul(a0, 0x0E) ^ mul(a1, 0x0B) ^ mul(a2, 0x0D) ^ mul(a3, 0x09)
                state[1][c] = mul(a0, 0x09) ^ mul(a1, 0x0E) ^ mul(a2, 0x0B) ^ mul(a3, 0x0D)
                state[2][c] = mul(a0, 0x0D) ^ mul(a1, 0x09) ^ mul(a2, 0x0E) ^ mul(a3, 0x0B)
                state[3][c] = mul(a0, 0x0B) ^ mul(a1, 0x0D) ^ mul(a2, 0x09) ^ mul(a3, 0x0E)

        shifts = [0, 1, 3, 4]
        new_state = [[0]*self.Nb for _ in range(4)]
        for r in range(4):
            s = shifts[r]
            for c in range(self.Nb):
                new_state[r][(c + s) % self.Nb] = state[r][c]
        state = new_state

        for r in range(4):
            for c in range(self.Nb):
                state[r][c] = INV_SBOX[state[r][c]]

        for r in range(4):
            for c in range(self.Nb):
                state[r][c] ^= self.w[c][r]

        out = bytearray(32)
        for r in range(4):
            for c in range(self.Nb):
                out[r + 4 * c] = state[r][c]
        return bytes(out)

# --- 主解密执行 ---
KEY_STR = "dXWXKKLrVLDgdXHmKmMdVReuepMXrqu4"
IV_STR  = "k3qCmcxzzWfdCHUUVvvmAaZGbQKanCUM"

key = KEY_STR.encode("utf-8")
iv  = IV_STR.encode("utf-8")

cipher = Rijndael256(key)

with open("masterdata_payload.enc", "rb") as f:
    enc_data = f.read()

print(f"[*] 密文大小: {len(enc_data)} 字节，开始 256 位块 CBC 解密...")

decrypted = bytearray()
prev = iv
for i in range(0, len(enc_data), 32):
    chunk = enc_data[i:i+32]
    d_chunk = cipher.decrypt_block(chunk)
    plain_chunk = bytes(a ^ b for a, b in zip(d_chunk, prev))
    decrypted.extend(plain_chunk)
    prev = chunk

raw_data = bytes(decrypted)

# 移除 PKCS7 填充
pad_len = raw_data[-1]
if 0 < pad_len <= 32 and all(b == pad_len for b in raw_data[-pad_len:]):
    unpadded = raw_data[:-pad_len]
    print(f"[*] 成功移除 PKCS7 填充 ({pad_len} 字节)")
else:
    unpadded = raw_data

print(f"[*] 解密后头部 32 字节: {unpadded[:32]}")

try:
    txt = unpadded.decode("utf-8").strip()
    if txt.startswith("{") or txt.startswith("["):
        with open("masterdata.json", "w", encoding="utf-8") as out:
            out.write(txt)
        print("[🎉🎉🎉] 成功解密为原始 JSON: masterdata.json")
        exit(0)
except Exception:
    pass

try:
    plain = gzip.decompress(unpadded)
    with open("masterdata.json", "wb") as out:
        out.write(plain)
    print("[🎉🎉🎉] 成功解密并解压 Gzip: masterdata.json")
    exit(0)
except Exception:
    pass

try:
    plain = zlib.decompress(unpadded)
    with open("masterdata.json", "wb") as out:
        out.write(plain)
    print("[🎉🎉🎉] 成功解密并解压 Zlib: masterdata.json")
    exit(0)
except Exception:
    pass

with open("masterdata_decrypted.bin", "wb") as out:
    out.write(unpadded)
print("[*] 结果已保存为 masterdata_decrypted.bin")
